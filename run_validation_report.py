...
def run_variant_experiment(variant_name: str, symbol: str, df: pd.DataFrame) -> Dict[str, Any]:
    ...
    try:
        ...
        # 2. Walk-Forward (3 rolling folds, 70% in-sample / 30% out-of-sample)
        wf = WalkForwardEngine(num_folds=3, in_sample_pct=0.70, initial_balance=10000.0, risk_per_trade_pct=0.5)
        ...
        return {
            ...
            "oos_trades": res_wf.get("total_oos_trades", 0),
...
def main():
    ...
    feed = DataFeedEngine(mt5_client=mt5_client)

    num_bars = 1000
    df_xau = feed.fetch_rates("XAUUSD", timeframe="H1", num_bars=num_bars)
    df_eur = feed.fetch_rates("EURUSD", timeframe="H1", num_bars=num_bars)
    ...
    for market_name, sym, df in [
        ("Trending Market (XAUUSD)", "XAUUSD", df_xau),
        ("Liquid / Mean-Reverting Market (EURUSD)", "EURUSD", df_eur)
    ]:
        source_label = df.attrs.get("data_source", "SYNTHETIC_FALLBACK")
        start_time = str(df.iloc[0]["time"]) if "time" in df.columns and len(df) > 0 else "N/A"
        end_time = str(df.iloc[-1]["time"]) if "time" in df.columns and len(df) > 0 else "N/A"

        print(f"\n[REGIME TEST] {market_name} [{len(df)} H1 Bars | {start_time} -> {end_time}]", flush=True)
        if source_label == "LIVE_MT5":
            print(f"[*] DATA SOURCE: LIVE MT5 HISTORICAL DATA (Server={getattr(mt5_client, 'server', 'MT5')})", flush=True)
        else:
            print("[!] WARNING: MT5 unavailable — Using SYNTHETIC fallback data (illustrative only)", flush=True)
